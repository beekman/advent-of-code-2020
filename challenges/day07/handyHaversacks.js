...
let numberOfMatchingBags = 0;

const willContainColor = (bags, color, matched) => {
...
for(color in allBags) {
  if(willContainColor(allBags[color], 'shiny gold', {})) {
    numberOfMatchingBags++;
  }
};

console.log(`There are ${numberOfMatchingBags} bags that have a shiny gold bag somewhere inside.`);

const numberOfNestedBags = howManyNestedBags('shiny gold');
console.log(`There are ${numberOfNestedBags} bags nested inside each shiny gold bag.`);
...
function extractRules(rules) {
  ...
  rules.forEach(rule => {
    ...
    if(parts[1]) {
      ...
      parts[0] = parts[0].replace((/ bags?/g), '');
      ...
      nestedBags.forEach(bag => {
        ...
        let subBagType = bagType.replace((/ bags?/g), '');
